...
class Re_Meta_Generator_checkbox
{
	public $post_id;
	public $type;
	public $field;
			...
			<div class="col-xs-8 up_meta_tail">
				...
				<?php if( $this->type == 'linear'): ?>
					...
					<?php foreach($field['options'] as $key => $value ){ ?>
						<input type="checkbox" name="<?php echo esc_attr( $meta_key );  ?>[]"  class="form-control" value="<?php echo esc_attr( $value['key'] );  ?>" <?php if(in_array($value['key'], $grabbed_value)){ ?> checked <?php } ?> ><?php echo esc_attr( $value['value'] );  ?><br>
					<?php } ?>
				...
				<?php if( $this->type == 'repeatBundle'): ?>
					<div class="row">
						<div class="col-xs-10">
							<?php foreach($field['options'] as $key => $value ){ ?>
								<input type="checkbox" name="<?php echo esc_attr( $meta_key );  ?>[]"  class="form-control" value="<?php echo esc_attr( $value['key'] );  ?>" <?php if(in_array($value['key'], $this->load)){ ?> checked <?php } ?> ><?php echo esc_attr( $value['value'] );  ?><br>
							<?php } ?>
						</div>
					</div>
				<?php endif; ?>
