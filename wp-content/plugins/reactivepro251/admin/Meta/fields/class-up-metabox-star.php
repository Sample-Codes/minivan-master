		...
		<div class="row meta-single-content">
			...
			<div class="col-xs-8 up_meta_tail">
				...
				<?php if( $this->type == 'linear'): ?>
					...
					<div class="cont">
					  	<div class="stars">
					  		<input type="hidden" name="<?php echo esc_attr( $meta_key ); ?>"  class="form-control linear-input-star-field" placeholder="<?php echo esc_attr( $placeholder );  ?>" value="<?php echo esc_attr( $grabbed_value );  ?>" >
							<div class="rate" data-rate-value=<?php echo esc_attr($grabbed_value); ?>></div>
					  	</div>
					</div>

					<p class="pull-right"><?php echo esc_attr( $desc );  ?></p><br>
				...
				<?php if( $this->type == 'repeatBundle'): ?>
					...
					<div class="row">
						<div class="col-xs-10">
							<div class="cont">
							  	<div class="stars">
							  		<input type="hidden" name="<?php echo esc_attr( $meta_key ); ?>[]"  class="form-control bundle-input-star-field" placeholder="<?php echo esc_attr( $placeholder );  ?>" value="<?php echo esc_attr($this->load);  ?>" >
									<div class="bundle-rate" data-rate-value=<?php echo esc_attr($this->load); ?>></div>
							  	</div>
							</div>
							<br>
						</div>
