...
class Re_Meta_Generator_select
{
	public $post_id;
	public $type;
	public $field;
			...
			<?php if( $this->type == 'linear'): ?>
				...
				<select name="<?php echo esc_attr( $meta_key ); ?>" id="<?php echo esc_attr( $meta_key ); ?>">
					...
					<?php foreach($field['options'] as $key => $value ){ ?>
						<option value="<?php echo esc_attr( $value['key'] );?>" <?php if($value['key'] == $grabbed_value){ ?> selected <?php } ?>><?php echo esc_attr( $value['value'] ); ?></option>
					<?php } ?>
				</select>
				...
				<div class="row">
					<div class="col-xs-10">
						<select name="<?php echo esc_attr( $meta_key ); ?>[]" id="<?php echo esc_attr( $meta_key ); ?>">
							<option value=""><?php _e('select','userplace'); ?></option>
							<?php foreach($field['options'] as $key => $value ){ ?>
								<option value="<?php echo esc_attr( $value['key'] );?>" <?php if($value['key'] == $this->load){ ?> selected <?php } ?>><?php echo esc_attr( $value['value'] ); ?></option>
							<?php } ?>
						</select>
					</div>
				</div>
