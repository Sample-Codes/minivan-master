...
<div id="price_calculation_product_data" class="panel woocommerce_options_panel">
    ...
    <?php
		woocommerce_wp_select( array( 'id' => 'pricing_type','label' => __( 'Set Price Type', 'redq-rental' ), 'description' => sprintf( __( 'Choose a price type - this controls the <a href="%s">schema</a>.', 'redq-rental' ), 'http://schema.org/' ), 'options' => array(
            ...
            'distance_pricing'  => __( 'Distance Pricing', 'redq-rental' ),
		) ) );
    ?>
    ...
    <div class="distance-pricing-panel show_if_distance_pricing">
        <h4 class="redq-headings">
            <?php _e('Set distance pricing plan','redq-rental'); ?>
        </h4>
        <?php
        woocommerce_wp_text_input( array( 'id' => 'distance_price', 'label' => __( 'Distance Price ( '.get_woocommerce_currency_symbol().' )', 'redq-rental' ), 'placeholder' => __( 'Enter price here', 'redq-rental' ), 'type' => 'text' ) );
        ?>
    </div>
